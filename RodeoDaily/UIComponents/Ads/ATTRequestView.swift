...
struct NotificationOnboardingView: View {
    @AppStorage("needsNotificationOnboarding") private var needsNotificationOnboarding = true
        ...
        GeometryReader { proxy in
            VStack {
                VStack(alignment: .leading, spacing: 30) {
                    ...
                    Text("Stay updated with follow alerts:")
                        .font(.title)
                        .fontWeight(.semibold)
                        .foregroundColor(.white)
                    ...
                    HStack(spacing: 20) {
                        ...
                        Text("Rank changes and new results for athletes you follow")
                            .font(.system(size: 20, weight: .medium))
                            .foregroundColor(.white)
                    }
    ...
    private func requestNotifications() {
        UNUserNotificationCenter.current().requestAuthorization(options: [.alert, .sound, .badge]) { granted, _ in
            ...
            DispatchQueue.main.async {
                needsNotificationOnboarding = false
            }
        }
    }
}
